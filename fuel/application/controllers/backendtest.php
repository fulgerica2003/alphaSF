	...
	class BackendTest extends CI_Controller
	{
		...
		*/
		...
		public function auth_code(){
			echo get_auth_code('ok');
		}
		
		public function compute_auth_code(){
			$hex_key = pack('H*', $this->config->item('encryption_key'));
			$input = "ok";
			
			$auth_code = strtoupper(hash_hmac('sha1', $input . date('YmdHi'), $hex_key));;
			//sleep(5);
			echo check_auth_code($input, $auth_code);
		}
	}			