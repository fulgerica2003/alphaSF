...
class Ss_exchange_rate_model extends Base_module_model {

	// read more about models in the user guide to get a list of all properties. Below is a subset of the most common:

	public $record_class = 'Ss_exchange_rate_item'; // the name of the record class (if it can't be determined)
	public $filters = array(); // filters to apply to when searching for items
	public $required = array('type', 'value', 'apply_date'); // an array of required fields. If a key => val is provided, the key is name of the field and the value is the error message to display
	...
	public $unique_fields = array(array('apply_date', 'type')); // fields that are not IDs but are unique. Can also be an array of arrays for compound keys
	...
	{
		parent::__construct('ss_exchange_rate'); // table name
	}
	...
	{	
		...
		$fields['apply_date'] += array('first_day' => 1, 'date_format' => 'Y-m-d', 'value' => date('Y-m-d'));
		return $fields;
	}
...
class Ss_exchange_rate_item_model extends Base_module_record {
