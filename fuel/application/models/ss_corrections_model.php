...
class Ss_corrections_model extends Base_module_model {

	// read more about models in the user guide to get a list of all properties. Below is a subset of the most common:

	public $record_class = 'Ss_correction'; // the name of the record class (if it can't be determined)
	...
	{
		parent::__construct('ss_corrections'); // table name
	}

	function list_items($limit = NULL, $offset = NULL, $col = 'precedence', $order = 'desc', $just_count = FALSE)
	{
		$this->db->select('ss_corrections.ben_name, ss_cities.name as city', FALSE);
		$this->db->join('ss_cities', 'ss_cities.id = ss_corrections.id_ben_city', 'left');
	...
	{
		$this->db->select('ss_corrections.*, ss_cities.name as ben_city', FALSE);
		$this->db->join('ss_cities', 'ss_cities.id = ss_corrections.id_ben_city', 'left');
		
		$data = parent::find_one_array($where, $order_by = null);
		return $data;
	}

}

class Ss_correction_model extends Base_module_record {
